...
export default function PlacesScreen() {
  ...
  useEffect(() => {
    // get places
      ...
      fetch(`http://192.168.0.53:3000/places/${user.nickname}`)
      .then(response => response.json())
      .then((data)=> {
        //console.log('datas',data)
        for(let one of data.places)
        {
          if(user.places.some((element) => element.name !== one.name) == false)
          {
            dispatch(addPlace(one))
          }
  ...
  const deletePlace = (name) => {
   ...
   fetch('http://192.168.0.53:3000/places', {
    method: 'DELETE',
  ...
  const places = user.places.map((data, i) => {
    return (
      <View key={i} style={styles.card}>
        ...
        <FontAwesome name='trash-o' onPress={() => deletePlace(data.name)} size={25} color='#ec6e5b' />
      </View>
    );
  });
